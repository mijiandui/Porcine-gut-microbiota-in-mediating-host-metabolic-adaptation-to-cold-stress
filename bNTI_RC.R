#### R codes for null model analysis modified according to 
#[1]Stegen et al. (2013) ####
#[2]Soil pH and temperature regulate assembly processes of abundant and rare bacterial communities inagricultural ecosystems
...
#############################################################################################
# ??NTI > 2: Community assembly driven by variable selection                                  #
# ??NTI < -2: Community assembly driven by homgenizing selection                              #
# |??NTI| < 2 & RCbray > 0.95: Community assembly driven by dispersal limitation              #
# |??NTI| < 2 & RCbray < -0.95: Community assembly driven by homogenizing dispersal           #
# |??NTI| < 2 & |RCbray| < 0.95: Community assembly is due to drift alone (or weak selection) #
#############################################################################################

#phylo: Phylogenetic tree of each OTU
#comun: A community table with OTUs as rows and samples as columns. 



phylo <- read.tree("tree.tre")
#comun <- read.table("otu_table_for_TNI.txt",header = TRUE,row.names = 1,sep = "\t")
#comun <- read.table("otu_table_Abundance_for_TNI.txt",header = TRUE,row.names = 1,sep = "\t")
comun <- read.table("otu_table_Inter_for_TNI.txt",header = TRUE,row.names = 1,sep = "\t")
#comun <- read.table("otu_table_rare_for_TNI.txt",header = TRUE,row.names = 1,sep = "\t")
comun<-t(comun)
...
bNTI <- as.data.frame(as.matrix(bNTI))
write.csv(bNTI, "bNTI_rare.csv", row.names = TRUE)
...
#RC <- raup_crick(comun, reps = 999)
RC <- raup_crick(comun, reps = 999)
RC <- as.data.frame(as.matrix(RC))
write.csv(RC, "RC-inter.csv", row.names = TRUE)
